...
void ResetDialog::on_verify_Btn_clicked()
{
    ...
    HttpMgr::GetInstance()->postHttpReg(QUrl(gate_url_prefix + "/get_verifycode"),
		json_obj, RegId::ID_GET_VERIFY_CODE, Modules::RESETMOD);
}

void ResetDialog::slot_reset_mod_finish(RegId id, QString res, ErrorCodes err)
{
...
{
    //注册获取验证码回包逻辑
    _handlers.insert(RegId::ID_GET_VERIFY_CODE, [this](QJsonObject jsonObj){
    ...
    //注册注册用户回包逻辑
    _handlers.insert(RegId::ID_RESET_PWD, [this](QJsonObject jsonObj){
        int error = jsonObj["error"].toInt();
        if(error != ErrorCodes::SUCCESS){
            showTip(tr("参数错误"),false);
            return;
...
void ResetDialog::on_sure_Btn_clicked()
{
    ...
    HttpMgr::GetInstance()->postHttpReg(QUrl(gate_url_prefix+"/reset_pwd"),
                 json_obj, RegId::ID_RESET_PWD,Modules::RESETMOD);
}
